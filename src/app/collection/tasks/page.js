...
import { Tooltip, Card, Form, Row, Col, Input, Select, Button, DatePicker, Space, Modal } from 'antd';
        ...
        <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
          <div className="tabs-bar" style={{ padding: '0 24px' }}>
            ...
            <div className="tabs-pill">
              <button className={`tab-pill ${activeTab === 'all' ? 'active' : ''}`} onClick={() => setActiveTab('all')}>📋 全部</button>
              <button className={`tab-pill ${activeTab === 'running' ? 'active' : ''}`} data-t="running" onClick={() => setActiveTab('running')}>⚡️ 进行中</button>
              <button className={`tab-pill ${activeTab === 'pending' ? 'active' : ''}`} data-t="pending" onClick={() => setActiveTab('pending')}>🕐 排队中</button>
              <button className={`tab-pill ${activeTab === 'done' ? 'active' : ''}`} data-t="done" onClick={() => setActiveTab('done')}>✅ 已完成</button>
              <button className={`tab-pill ${activeTab === 'error' ? 'active' : ''}`} data-t="error" onClick={() => setActiveTab('error')}>⚠️ 异常</button>
            </div>
          </div>

          <div className="toolbar" style={{ padding: '16px 24px' }}>
            <div className="toolbar-left"><span className="tbl-title">数据采集列表</span></div>
                    ...
                    <tr key={idx}>
                      ...
                      <td style={{ position: 'sticky', right: 0, background: '#fff' }}>
                        <div className="act-btns">
                          ...
                          <span className="act-sep">|</span>
                          <button className="act-btn">复制</button>
                          <span className="act-sep">|</span>
